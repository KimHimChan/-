...
<body>
	...
	<h1 class="name_room"><u>В Великом Новгороде на каникулах побывали почти 30 тыс. туристов</u></h1>
	<?
	...
	<div class="pages">
		<?
			...
			$result = mysql_query("SELECT * FROM coment WHERE id_page = '2'");

			for ($i = 0; $i < mysql_num_rows($result); $i++) { 
				$rez = mysql_fetch_array($result);
				echo "<form method='post' action='comment_del_2.php' class='commentaries'>";
		...
		<?
			if (isset($_SESSION['login'])) {
				if (empty($result_query['login'])) {
					...
					echo "<input type='hidden' name='page_id' value='2'>";
