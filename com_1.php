...
<body>
	...
	<h1 class="name_room"><u>Госдума одобрила запрет организовывать паломнические поездки</u></h1>
	<?
	...
	<div class="pages">
		<?
			...
			$result = mysql_query("SELECT * FROM coment WHERE id_page = '1'");

			for ($i = 0; $i < mysql_num_rows($result); $i++) { 
				$rez = mysql_fetch_array($result);
				echo "<form method='post' action='comment_del.php' class='commentaries'>";
		...
		<?
			if (isset($_SESSION['login'])) {
				if (empty($result_query['login'])) {
					...
					echo "<input type='hidden' name='page_id' value='1'>";
